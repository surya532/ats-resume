...
const EXPAND_ICON   = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>`;
const COLLAPSE_ICON = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="4 14 10 14 10 20"/><polyline points="20 10 14 10 14 4"/><line x1="10" y1="14" x2="3" y2="21"/><line x1="21" y1="3" x2="14" y2="10"/></svg>`;

function toggleInputsExpand() {
  const expanded = document.body.classList.toggle('inputs-full');
  const btn = document.getElementById('expandInputsBtn');
  if (!btn) return;
  btn.classList.toggle('active', expanded);
  btn.title     = expanded ? 'Collapse inputs' : 'Expand inputs';
  btn.innerHTML = expanded ? COLLAPSE_ICON : EXPAND_ICON;
}

function collapseInputs() {
  document.body.classList.remove('inputs-full');
  const btn = document.getElementById('expandInputsBtn');
  if (btn) { btn.classList.remove('active'); btn.title = 'Expand inputs'; btn.innerHTML = EXPAND_ICON; }
}
